...
namespace Basket.API.Data
{
    public class CachedBasketRepository(IBasketRepository basketRepository, IDistributedCache cache) : IBasketRepository
    {
        ...
        {
            var checchedBasket = await cache.GetStringAsync(userName, cancellationToken);
            if (!string.IsNullOrEmpty(checchedBasket))
            {
              return  JsonSerializer.Deserialize<ShoppingCart>(checchedBasket);
            }
